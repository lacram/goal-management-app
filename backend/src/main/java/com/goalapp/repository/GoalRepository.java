...
public interface GoalRepository extends JpaRepository<Goal, Long> {
    ...
    // findAll은 EntityGraph 제거 (메모리 절약)
    // 필요시 findByParentGoalIsNull() 사용
    @Override
    List<Goal> findAll();

    // findById는 EntityGraph 유지 (개별 조회 시 하위 목표 필요)
    ...
    // 특정 타입의 목표들 조회 (EntityGraph 제거 - 메모리 절약)
    List<Goal> findByType(GoalType type);

    // 특정 상태의 목표들 조회 (EntityGraph 제거 - 메모리 절약)
    List<Goal> findByStatus(GoalStatus status);

    // 특정 타입과 상태로 조회 (EntityGraph 제거 - 메모리 절약)
    ...
    // 마감일이 지난 미완료 목표들 조회 (EntityGraph 제거)
    ...
    // 특정 기간 내 생성된 목표들 조회 (EntityGraph 제거)
    ...
    // 우선순위별 정렬된 목표들 조회 (EntityGraph 유지 - 루트 목표용)
    @EntityGraph(attributePaths = {"subGoals"})
    List<Goal> findByParentGoalIsNullOrderByPriorityDesc();

    // 모든 목표를 우선순위 순으로 조회 (EntityGraph 제거)
    ...
    // 오늘의 목표들 조회 (EntityGraph 제거 - 메모리 절약)
    ...
    // 만료된 목표 조회 (EntityGraph 제거 - 메모리 절약)
    ...
    // 만료 임박 목표 조회 (EntityGraph 제거 - 메모리 절약)
    ...
    // EXPIRED 상태이고 일정 시간 경과한 목표 조회 (EntityGraph 제거)
    ...
    // 완료된 지 일정 시간 경과한 목표 조회 (EntityGraph 제거)
